...
async def create_image_prompt(user_input, character, text_api):
    ...
    subject = user_input.split('of', 1)[1] if "of" in user_input else character + "Please describe yourself in vivid detail."
...
async def add_to_conversation_history(message, userName, file, user):
    # Add message to user's conversation history
    file_name = get_file_name("context", f"{user.name}.txt")
...
async def clean_llm_reply(message, userName, bot):
    ...
    pattern = re.compile(re.escape(bot_lower) + r":|" + re.escape(userName_lower) + r":|You:|#bots", re.IGNORECASE)
